...
class Task {
 public:
  ...
  Task(std::string taskName, int stackSize, int priority, int core,
    ...
    if (runOnPSRAM) {
      // Callers pass stackSize in bytes. ESP-IDF FreeRTOS treats
      // xTaskCreateStatic* depth as StackType_t words (despite some docs).
  ...
  bool startTask() {
    ...
    if (runOnPSRAM) {
      if (this->xStack == NULL) {
        printf("BellTask: PSRAM stack alloc failed for %s\n", this->TASK.c_str());
        return false;
      }
      xTaskBuffer = (StaticTask_t*)heap_caps_malloc(
          sizeof(StaticTask_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
      // Convert byte size from constructors to FreeRTOS word depth.
      const uint32_t stackDepthWords =
          (uint32_t)(this->stackSize / sizeof(StackType_t));
      return (xTaskCreateStaticPinnedToCore(
                  taskEntryFuncPSRAM, this->TASK.c_str(), stackDepthWords, this,
