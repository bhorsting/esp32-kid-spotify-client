...
std::vector<uint8_t> LoginBlob::decodeBlob(
  ...
  if (blob.size() < 36) {
    CSPOT_LOG(error, "decodeBlob: payload too short (%u)", (unsigned)blob.size());
    return {};
  }
  ...
  // Check checksum — abort on mismatch (continuing decrypts garbage / crashes).
  if (mac != checksum) {
...
std::vector<uint8_t> LoginBlob::decodeBlobSecondary(
  ...
  /* size_t underflow: `i < l - 16` wraps when l < 16 and walks off the buffer. */
  if (blobData.size() < 16) {
    CSPOT_LOG(error, "decodeBlobSecondary: blob too short (%u)",
              (unsigned)blobData.size());
    return {};
  }
  const size_t l = blobData.size();
...
void LoginBlob::loadZeroconf(const std::vector<uint8_t>& blob,
  ...
  if (partDecoded.empty()) {
    CSPOT_LOG(error, "loadZeroconf: primary blob decode failed");
    return;
  }
  auto loginData = this->decodeBlobSecondary(partDecoded, username, deviceId);
  if (loginData.size() < 16) {
    CSPOT_LOG(error, "loadZeroconf: secondary blob decode failed");
    return;
  }
  ...
  if (blobSkipPosition + authSize > loginData.size()) {
    CSPOT_LOG(error, "loadZeroconf: authData out of range");
    return;
  }
