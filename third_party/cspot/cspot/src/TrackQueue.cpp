...
size_t TrackQueue::snapshotFromCurrent(std::vector<SnapshotEntry>& out,
                                       size_t max) {
  std::scoped_lock lock(tracksMutex);
  out.clear();
  if (max == 0 || currentTracksIndex < 0 || currentTracks.empty() ||
      currentTracksIndex >= (int16_t)currentTracks.size()) {
    return 0;
  }

  const size_t total = currentTracks.size();
  /* Wrap so the ring always has up to `max` entries, even near playlist end. */
  const size_t take = max < total ? max : total;
  for (size_t n = 0; n < take; n++) {
    size_t idx = ((size_t)currentTracksIndex + n) % total;
    SnapshotEntry entry;
    entry.ref = currentTracks[idx];
    entry.hasMeta = false;
    entry.absIndex = (int)idx;
    for (auto& pt : preloadedTracks) {
      if (pt && pt->ref == currentTracks[idx] && !pt->trackInfo.name.empty()) {
        entry.info = pt->trackInfo;
        entry.hasMeta = true;
        break;
      }
    }
...
static bool refMatchesTrackId(const TrackReference& ref, const std::string& trackId) {
  if (!ref.uri.empty()) {
    auto pos = ref.uri.rfind(':');
    if (pos != std::string::npos && ref.uri.substr(pos + 1) == trackId) {
      return true;
    }
  }
  if (ref.gid.empty()) {
  ...
  auto hex = bytesToHexString(ref.gid);
  if (hex == trackId) {
    return true;
  }
  /* Also compare base62 form of GID. */
  static const char* alpha =
      "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  std::vector<uint8_t> num = ref.gid;
  std::string b62;
  while (!num.empty()) {
    bool all_zero = true;
    for (uint8_t b : num) {
    ...
    b62.insert(b62.begin(), alpha[rem]);
    num.swap(div);
  }
  while (b62.size() < 22) {
    b62.insert(b62.begin(), '0');
  }
  return b62 == trackId;
}

int TrackQueue::findIndexByTrackId(const std::string& trackId) {
  if (trackId.empty()) {
    return -1;
  }
  std::scoped_lock lock(tracksMutex);
  ...
  const size_t total = currentTracks.size();
  /* Prefer upcoming (from current) so playlist duplicates don't jump backwards. */
  const size_t start =
      (currentTracksIndex >= 0 && (size_t)currentTracksIndex < total)
          ? (size_t)currentTracksIndex
          : 0;
  for (size_t n = 0; n < total; n++) {
    size_t i = (start + n) % total;
    if (refMatchesTrackId(currentTracks[i], trackId)) {
...
bool TrackQueue::jumpRelative(int offset) {
  if (offset == 0) {
    return true;
  }
  if (offset < 0) {
  ...
  {
    std::scoped_lock lock(tracksMutex);
    if (currentTracks.empty() || currentTracksIndex < 0 ||
        currentTracksIndex >= (int16_t)currentTracks.size()) {
      return false;
    }
    const size_t total = currentTracks.size();
    target = (int16_t)(((size_t)currentTracksIndex + (size_t)offset) % total);
  }
...
bool TrackQueue::jumpToIndex(int16_t index) {
  std::scoped_lock lock(tracksMutex);
  if (index < 0 || index >= (int16_t)currentTracks.size()) {
    return false;
  }
  if (index == currentTracksIndex && !preloadedTracks.empty()) {
    return true;
  }
  ...
  CSPOT_LOG(info, "Jumped to track index %d", (int)index);
  return true;
}
...
void TrackQueue::processTrack(std::shared_ptr<QueuedTrack> track) {
  switch (track->state) {
    ...
    case QueuedTrack::State::KEY_REQUIRED: {
      /* Only fetch audio for the playing head + one lookahead.
       * Further preloads stop after metadata so the ring can show covers
       * without opening extra CDN HTTP streams (which starve Wi‑Fi). */
      size_t idx = 0;
      {
        std::scoped_lock lock(tracksMutex);
        auto it =
            std::find(preloadedTracks.begin(), preloadedTracks.end(), track);
        if (it == preloadedTracks.end()) {
          break;
        }
        idx = (size_t)(it - preloadedTracks.begin());
      }
      if (idx >= (size_t)MAX_TRACKS_AUDIO) {
        /* Mark failed for audio purposes but keep trackInfo for UI. */
    ...
    case QueuedTrack::State::CDN_REQUIRED:
      ...
      break;
    case QueuedTrack::State::FAILED: {
      /* Meta-only preloads become FAILED; revive when they enter the audio window. */
      size_t idx = 0;
      bool found = false;
      {
        std::scoped_lock lock(tracksMutex);
      ...
      if (found && idx < (size_t)MAX_TRACKS_AUDIO &&
          !track->trackInfo.name.empty()) {
        track->state = QueuedTrack::State::KEY_REQUIRED;
      }
      break;
    }
    default:
